    ...
    woocommerce_wp_select(
        array(
            ...
            'options' => array(
                ...
                'пор' => __('пор', 'productunit'),
                'пач' => __('пач', 'productunit'),
                'кг' => __('кг', 'productunit'),
            )
        )
    );
    echo '</div>';
...
function woo_add_custom_general_fields_save($post_id)
{
    ...
    // Select
    $woocommerce_select = $_POST['_productunit'];
    if (!empty($woocommerce_select))
        update_post_meta($post_id, '_productunit', esc_attr($woocommerce_select));

}

add_action("wp_ajax_vnupdatecart", "vn_update_cart1");
add_action("wp_ajax_nopriv_vnupdatecart", "vn_update_cart1");
function vn_update_cart1()
{
...
function my_action_javascript()
{
    ...
    <script type="text/javascript">
        jQuery(document).ready(function ($) {
            ...
            // $(".shipping-calculator-form :nth-child(2)").attr("selected", "selected");
            $(".shipping-calculator-button").trigger("click");
            $(document).on('change', '[name^="shipping_method"]', function (e) {
                //$('body').trigger('update_checkout'); // Trigger Ajax checkout refresh event
                // var val_fake = parseInt($('input[name^="fake-input"]').val());
                //
                // $('input[name^="fake-input"]').val(val_fake + 1 );
                // console.log($('input[name^="fake-input"]').val())
                ...
                $.ajax(
                    {
                        ...
                        type: "POST",
                        data: {
                            action: "vnupdatecart", 'shipping_method[0]': shipping_method ,
                            security:wc_cart_params.update_shipping_method_nonce
                        },
                        success: function (data) {
                            $("[name^='update_cart']").prop( "disabled", null );
